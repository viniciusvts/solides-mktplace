...
// get posts dropdown
function themeplace_get_portfolio_dropdown_array($args = [], $key = 'ID', $value = 'post_title') {
  $options = [];
...
function themeplace_add_elementor_widget_categories( $elements_manager ) {

	$elements_manager->add_category(
		'themeplace-elements',
		[
			'title' => esc_html__( 'themeplace Elements', 'themeplace' ),
			'icon' => 'fa fa-plug',
		]
	);

}
add_action( 'elementor/elements/categories_registered', 'themeplace_add_elementor_widget_categories' );

//Elementor init

class themeplace_ElementorCustomElement {
...
themeplace_ElementorCustomElement::get_instance()->init();